	...
	implements OnInit
{
		...
		this.campaignService
			...
			?.subscribe((result) => {
				if (result.responseStatus === 200) {
					...
					this.campaignSpecialGifts =
						result.content.datas.campaignSpecialGifts;
	...
	doUpdateCampaignInfo() {
		...
		this.currentStartDate = `${this.campaignStartDate.getFullYear()}-${
			this.campaignStartDate.getMonth() + 1
		}-${this.campaignStartDate.getDate()}`;
		this.currentEndDate = `${this.campaignEndDate.getFullYear()}-${
			this.campaignEndDate.getMonth() + 1
		}-${this.campaignEndDate.getDate()}`;
		this.currentRedemptionEndDate = `${this.campaignRedemptionEndDate.getFullYear()}-${
			this.campaignRedemptionEndDate.getMonth() + 1
	...
	getStartDate($event: any) {
		let d = new Date(Date.parse($event));
		this.currentStartDate = `${d.getFullYear()}-${
			d.getMonth() + 1
		}-${d.getDate()}`;
	}
	...
	getEndDate($event: any) {
		let d = new Date(Date.parse($event));
		this.currentEndDate = `${d.getFullYear()}-${
			d.getMonth() + 1
		}-${d.getDate()}`;
	}
	...
	getRedemptionEndDate($event: any) {
		let d = new Date(Date.parse($event));
		this.currentRedemptionEndDate = `${d.getFullYear()}-${
			d.getMonth() + 1
		}-${d.getDate()}`;
	}
}
